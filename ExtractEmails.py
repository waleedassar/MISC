...
def FindInList(ListX,Element):
    for tEle in ListX:
        if tEle.lower()==Element:
            return True
    return False
...
def Extract(iFile):
    if os.path.exists(iFile)==False:
        ...
        if Length == 0:
            ...
            iFileX = open(iFile,"r")
            for f in iFileX:
                XXxxXX = re.findall("([a-zA-Z0-9+._-]+@[a-zA-Z0-9_]+\.[a-zA-Z0-9.]+)",f)
                if XXxxXX:
                    NumEmails = len(XXxxXX)
                    if NumEmails != 0:
                        for Email in XXxxXX:
                            print Email
                            liDots = Email.split(".")
                            LastSeg = liDots[-1]
                            if FindInList(TLD,LastSeg)==True:
                                fOut.write(EmailX + "\r\n")
            iFileX.close()
            fOut.close()
            return 0
...
if len(sys.argv) != 2:
    print "Usage: ExtractEmails.py input_file\r\n"
    sys.exit(-1)
else:
